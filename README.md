...
- **AI order anomaly triage** (discipline 8) — optional post-commit consumer on `orders.created` (`ORDER_ANOMALY_TRIAGE_ENABLED`); Spring AI classify only, LLM **outside** any DB transaction, idempotent on `order_id`; never on checkout TX
